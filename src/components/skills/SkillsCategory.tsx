...
const SkillsCategoryPage: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        return category ? skillsDiagram.filter(s => s.category === category) : [];
    }, [category]);

    const data = useMemo(() => {
        if (!activeConfig || filtered.length === 0) return { labels: [], datasets: [] };
        return {
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black py-16 px-6">
            <div className="max-w-6xl mx-auto">
                ...
                <motion.h2
                    ...
                    className={`text-5xl md:text-5xl font-black text-center mb-4 bg-gradient-to-r ${activeConfig.gradient} bg-clip-text text-transparent`}
                >
                    {activeConfig.title}
                </motion.h2>
